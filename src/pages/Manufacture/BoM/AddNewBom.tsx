...
export default function AddNewBom() {
  ...
  const handleDeleteOfcItem = (id) => {
    setOfcItems((prev) => prev.filter((i) => i.id !== id));
  };
  ...
  const handleDeleteUtilitiesItem = (id) => {
    setUtilitiesItems((prev) => prev.filter((i) => i.id !== id));
  };
  ...
  const handleDeleteDepreciationItem = (id) => {
    setDeprItems((prev) => prev.filter((item) => item.id !== id));
  };
  ...
  const handleIndirectLaborChange = (id, field, value) => {
    setIndirectLaborItems((prev) =>
      prev.map((item) => {
        ...
        // --- HITUNG RATE/DAY ---
        if (updated.workingDay > 0) {
          updated.rateDay = updated.rateMonth / updated.workingDay;
        } else {
          updated.rateDay = 0;
        }

        // --- HITUNG RATE/HOURS ---
        if (updated.workingHours > 0) {
          updated.rateHours = updated.rateDay / updated.workingHours;
        } else {
          updated.rateHours = 0;
        }
        ...
        if (hasAll) {
          ...
          updated.rateEstimated = updated.qty * updated.rateMonth;
        }
  ...
  const handleDeleteIndirectLaborItem = (id) => {
    setIndirectLaborItems((prev) => prev.filter((item) => item.id !== id));
  };
  ...
  const handleLaborChange = (
    ...
    setLaborItems((prev) =>
      prev.map((item) => {
        ...
        // OrderCompletion (Days)
        updated.orderCompletionDays =
          workingHours > 0 ? orderCompletion / workingHours : 0;

        // Rate-Estimated
        const useHoursCalc =
          workingDay > 0 && workingHours > 0 && orderCompletion > 0;

        updated.rateEstimated = useHoursCalc
        ...
        <div className="p-6 space-y-8">
          ...
          <Card>
            <CardContent className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <p className="font-medium mb-1">Process 1 Name</p>
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 space-y-4 border mt-10">
                ...
                <div className="flex justify-end pt-4">
                  <div className="text-right">
                    <p className="font-semibold text-gray-700">
                      Total Factory Plant/Machine/Equipment
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                        ...
                        <div>
                          ...
                          <p className="font-semibold">
                            {item.ofcEstimatedQty.toLocaleString()}{" "}
                            {item.ofcUnit}
                          </p>
                        </div>
                      </div>
