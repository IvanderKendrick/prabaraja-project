...
// Approval API response interface
...
export type ApprovalAction = 
  | 'getApprovalQuotation'
  | 'getApprovalRequest'
  | 'getApprovalShipment'
  | 'getApprovalInvoice'
  | 'getApprovalBillingInvoice';

// Generic hook for fetching approvals by action
export const useApprovals = (action: ApprovalAction, refreshKey?: number) => {
  const [data, setData] = useState<ApprovalAPIResponse[]>([]);
  ...
  const fetchApprovals = async (currentPage: number, currentLimit: number) => {
    try {
      setIsLoading(true);
      setError(null);
      ...
      // approvals: fetch by action={action}
  ...
  // Fetch data when page, limit, action, or refreshKey changes
  useEffect(() => {
    fetchApprovals(page, limit);
  }, [page, limit, action, refreshKey]);
