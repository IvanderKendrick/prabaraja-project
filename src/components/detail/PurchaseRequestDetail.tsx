...
export function PurchaseRequestDetail() {
  ...
  const handleDownloadAttachment = () => {
    if (request?.attachment_url) {
      const url = Array.isArray(request.attachment_url) ? request.attachment_url[0] : request.attachment_url.replace(/[\[\]"]/g, "");
  ...
  if (error)
    return (
      <div className="flex flex-col items-center justify-center min-h-screen space-y-4">
        <h2 className="text-2xl font-semibold text-gray-800">Failed to load request</h2>
  ...
  if (!request)
    return (
      <div className="flex flex-col items-center justify-center min-h-screen space-y-4">
        <h2 className="text-2xl font-semibold text-gray-800">Request Not Found</h2>
        <p className="text-gray-500">The request you are looking for doesn’t exist or was deleted.</p>
        <Button onClick={handleGoBack} variant="outline">
          <ArrowLeft className="mr-2 h-4 w-4" />
          Go Back
        </Button>
  ...
  const daysUntilExpiry = Math.ceil((new Date(request.due_date).getTime() - new Date().getTime()) / (1000 * 3600 * 24));
  const displayNumber = request?.number ? (request.number.toString().startsWith("REQ-") ? request.number : `REQ-${request.number}`) : "";

  return (
    <div className="flex h-screen overflow-hidden">
      <Sidebar />
      <div className="flex-1 overflow-auto">
        <Header title={`Purchase Request ${displayNumber}`} description="View and manage details of this purchase request" />
        ...
        <div className="container mx-auto p-6 space-y-6">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Badge className={request.status === "Approved" ? "bg-green-100 text-green-800" : request.status === "Rejected" ? "bg-red-100 text-red-800" : isExpired ? "bg-gray-100 text-gray-800" : "bg-yellow-100 text-yellow-800"}>
