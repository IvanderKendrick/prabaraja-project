...
export function PurchaseOrderDetail() {
  ...
  const handleDownloadAttachment = () => {
    if (order?.attachment_url) {
      const url = Array.isArray(order.attachment_url) ? order.attachment_url[0] : order.attachment_url.replace(/[\[\]"]/g, "");
  ...
  const isExpired = new Date() > new Date(order.due_date);
  const daysUntilExpiry = Math.ceil((new Date(order.due_date).getTime() - new Date().getTime()) / (1000 * 3600 * 24));
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex-1 overflow-auto">
        <Header title={`Purchase Order ${displayNumber}`} description="View and manage details of this purchase order" />
        ...
        <div className="container mx-auto p-6 space-y-6">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Badge className={order.status === "Approved" ? "bg-green-100 text-green-800" : order.status === "Rejected" ? "bg-red-100 text-red-800" : isExpired ? "bg-gray-100 text-gray-800" : "bg-yellow-100 text-yellow-800"}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-6 lg:col-span-2">
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <Package className="h-5 w-5" /> Ordered Items
                  </CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                ...
                <CardContent className="text-sm space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span>{formatCurrency(order.installment_amount || 0)}</span>
                  </div>
