...
export function ShipmentsTable({ onDelete, onEdit, onView }: ShipmentsTableProps) {
  const { data: apiData, isLoading, error, page, limit, totalPages, total, handlePageChange, handleLimitChange, refresh } = useShipmentsAPI();
            ...
            {shipments.length === 0 ? (
              ...
              shipments.map((shipment) => (
                <TableRow key={shipment.id}>
                  <TableCell className="font-medium">{formatDate(shipment.date)}</TableCell>
                  <TableCell>
                    <button onClick={() => (onView ? onView(shipment.id) : (window.location.href = `/purchase-shipment/${shipment.id}`))} className="text-blue-600 hover:text-blue-800 hover:underline font-medium">
                  ...
                  <TableCell>
                    <Badge className={getStatusBadgeProps(shipment.status)}>{shipment.status.charAt(0).toUpperCase() + shipment.status.slice(1)}</Badge>
                  </TableCell>
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="bg-white">
                        ...
                        {onDelete && (
                          <DropdownMenuItem onClick={() => handleDeleteClick(shipment.id)} className="text-red-600">
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This action cannot be undone. This will permanently delete the shipment.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
